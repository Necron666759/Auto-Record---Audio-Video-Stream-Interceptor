...
Единый источник правды о том, какие контейнеры доступны для какого сервиса
(Suno / YouTube), как они выглядят в блочном меню (подпись, цвет) и какими
аргументами ffmpeg кодируется каждый из них.

Windows-редакция: содержимое совпадает с Linux-версией (аудио- И видео-
контейнеры, правила «звук внутри видео»), отличается только «сырой» формат
захвата: WASAPI отдаёт float32 движка Windows, а не s24le из parec, поэтому
констант RAW_CAPTURE_PAREC_* здесь нет (см. encode_helpers.ffmpeg_raw_wav_args_dynamic).

Используется:
  - recorder_gui.py            — рисует блочное меню из containers_for_service()
  - auto_record_suno.py        — берёт audio-контейнер по имени (all_containers())
  - auto_record_youtube.py     — то же для audio, плюс video_codec_args() и
                                  правила звука внутри видео для video-контейнеров

Требования по качеству (те же, что в Linux-версии):
  - Исходная (сырая) запись ВСЕГДА идёт как WAV 96 кГц / 24 бит (PCM s24le, стерео).
  - Если пользователь НЕ выбрал контейнер — итоговый файл остаётся WAV 96 кГц / 24 бит.
  - Lossy-контейнеры (mp3/aac/ogg) — 320 kbit CBR, 48 кГц (потолок формата).
  - FLAC — lossless, сохраняет полные 96 кГц / 24 бит.
  - Для видео: звук внутри видео остаётся WAV 96kHz/24bit там, где контейнер это
    физически поддерживает (mp4/mkv/avi/mov); для webm/flv/3gp — лучший
    практически доступный вариант (см. video_default_audio_reason).
"""
...
# --------------------------------------------------------------------------
# Качество "сырой" записи (до выбора контейнера) — как в Linux-версии
# --------------------------------------------------------------------------
RAW_CAPTURE_RATE = 96000            # Гц
RAW_CAPTURE_BIT_DEPTH = 24          # бит
RAW_CAPTURE_WAV_CODEC = "pcm_s24le" # кодек WAV-файла записи
...
def _aac_args() -> list[str]:
    ...
    # ffmpeg по умолчанию. Выбор энкодера делает best_aac_encoder()
...
def _ogg_args() -> list[str]:
    # libvorbis: -q:a 10 — максимальное доступное качество.
    return [
...
def all_containers() -> list[ContainerOption]:
    """Список аудио-контейнеров (mp3/aac/ogg/flac). Используется CLI Suno-бэкенда
    (choices у --container) — видео-контейнеры туда не относятся."""
    return list(AUDIO_CONTAINERS)
