#!/usr/bin/env python3
"""
i18n.py (Windows редакция)

Интерфейсные строки на русском/английском. Никакой магии: словарь "ключ ->
текст" на каждый язык плюс функция tr(lang, key, **kwargs) с подстановкой через
str.format. Заголовок окна (app_title) — это название программы, поэтому оно не
переводится и одинаково на обоих языках.

Набор строк совпадает с Linux-версией (сервисы Suno и YouTube, блоки «Аудио» и
«Видео», «Звук внутри видео»), кроме:
  - убраны строки «Захват экрана: Авто/X11/Wayland» — на Windows нет выбора
    сервера отображения (захват экрана всегда через gdigrab);
  - добавлена строка чекбокса аварийного режима захвата звука без изоляции по
    процессу (capture_fallback_label);
  - тексты про firefox.exe / диагностику звука — под Windows.
"""
...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    "ru": {
        ...
        "choose_btn": "Выбрать...",
        "capture_fallback_label": "Аварийный режим: писать весь звук системы (без изоляции по Firefox)",
        ...
        "reset_firefox_cache_confirm": "Сохранённая копия профиля Firefox и путь к firefox.exe "
                                        "будут удалены. При следующем запуске они будут определены заново. Продолжить?",
        ...
        "audio_check_timeout": "\n[проверка звука не уложилась в {sec} с и была прервана]\n",
        "worker_no_output": "\n[процесс не вывел ни одной строки — если это неожиданно, "
                             "посмотрите файл журнала: {path}]\n",
    },
    "en": {
        "app_title": APP_TITLE,
        ...
        "choose_btn": "Browse...",
        "capture_fallback_label": "Fallback mode: record all system audio (no Firefox isolation)",
        ...
        "reset_firefox_cache_confirm": "The saved Firefox profile copy and the remembered "
                                        "firefox.exe path will be deleted. They will be "
                                        "re-detected on the next run. Continue?",
        ...
        "audio_check_timeout": "\n[the audio check did not finish within {sec} s and was aborted]\n",
        "worker_no_output": "\n[the process printed nothing - if that is unexpected, "
                             "see the log file: {path}]\n",
    },
}
